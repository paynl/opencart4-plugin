...
$request->setAmount(5.3);
$request->setCurrency('EUR');
$request->setPaymentMethodId(10); # iDEAL
...
echo 'getPaymentUrl: ' . '<a href="' . $transaction->getPaymentUrl() . '">' . $transaction->getPaymentUrl() . '</a>' . PHP_EOL;
...
echo 'expire: ' . $transaction->getExpire() . ' - ' . date('Y-m-d H:i:s', $transaction->getExpire()) . PHP_EOL;
echo 'expiresAt: ' . $transaction->getExpiresAt() . PHP_EOL;
echo 'created: ' . $transaction->getCreated() . PHP_EOL;
...
echo 'modified: ' . $transaction->getModified() . PHP_EOL;
