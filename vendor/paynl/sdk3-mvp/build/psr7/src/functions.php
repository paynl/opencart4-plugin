...
use PayNL\Sdk\Packages\Psr\Http\Message\MessageInterface;
use PayNL\Sdk\Packages\Psr\Http\Message\RequestInterface;
use PayNL\Sdk\Packages\Psr\Http\Message\ResponseInterface;
use PayNL\Sdk\Packages\Psr\Http\Message\ServerRequestInterface;
 ...
 * @return string
 */
function str(\PayNL\Sdk\Packages\Psr\Http\Message\MessageInterface $message)
{
    if ($message instanceof \PayNL\Sdk\Packages\Psr\Http\Message\RequestInterface) {
        $msg = \trim($message->getMethod() . ' ' . $message->getRequestTarget()) . ' HTTP/' . $message->getProtocolVersion();
        if (!$message->hasHeader('host')) {
            $msg .= "\r\nHost: " . $message->getUri()->getHost();
        }
    } elseif ($message instanceof \PayNL\Sdk\Packages\Psr\Http\Message\ResponseInterface) {
 ...
 *
 * This function accepts a string or {@see \PayNL\Sdk\Packages\Psr\Http\Message\UriInterface} and
...
{
    if ($uri instanceof \PayNL\Sdk\Packages\Psr\Http\Message\UriInterface) {
        return $uri;
...
function stream_for($resource = '', array $options = [])
{
    ...
    switch (\gettype($resource)) {
        ...
        case 'object':
            if ($resource instanceof \PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface) {
                return $resource;
 ...
 */
function modify_request(\PayNL\Sdk\Packages\Psr\Http\Message\RequestInterface $request, array $changes)
{
    if (!$changes) {
        return $request;
    }
    ...
    if ($request instanceof \PayNL\Sdk\Packages\Psr\Http\Message\ServerRequestInterface) {
 ...
 */
function rewind_body(\PayNL\Sdk\Packages\Psr\Http\Message\MessageInterface $message)
{
 ...
 */
function copy_to_string(\PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface $stream, $maxLen = -1)
{
    $buffer = '';
    if ($maxLen === -1) {
        while (!$stream->eof()) {
 ...
 * @throws \RuntimeException on error.
 */
function copy_to_stream(\PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface $source, \PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface $dest, $maxLen = -1)
{
    $bufferSize = 8192;
    if ($maxLen === -1) {
        while (!$source->eof()) {
 ...
 * @throws \RuntimeException on error.
 */
function hash(\PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface $stream, $algo, $rawOutput = \false)
{
    $pos = $stream->tell();
    if ($pos > 0) {
        $stream->rewind();
 ...
 * @return string
 */
function readline(\PayNL\Sdk\Packages\Psr\Http\Message\StreamInterface $stream, $maxLength = null)
{
    $buffer = '';
    $size = 0;
    while (!$stream->eof()) {
 ...
 * @return null|string
 */
function get_message_body_summary(\PayNL\Sdk\Packages\Psr\Http\Message\MessageInterface $message, $truncateAt = 120)
{
    $body = $message->getBody();
